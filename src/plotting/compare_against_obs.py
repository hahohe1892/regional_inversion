...
for RID_obs in RIDs_with_obs[:4]:
    ...
    out_to_tif(RID, 'smb', i = 0, file = 'ex.nc', file_not_standard_dims = True)
    ...
    glathida_NO['THICK_Millan'][glathida_NO['THICK_Millan'] < 0] = np.nan
    ...
    glathida_NO['THICK_OBS_CORR'][glathida_NO['THICK_OBS_CORR'] < 0] = np.nan
...
ax.scatter(reference_thickness, all_NO.THICK_Millan, alpha = .2, label = 'Millan (2022)')
ax.scatter(reference_thickness, all_NO.THICK_CONS, alpha = .2, label = 'Consensus')
ax.scatter(reference_thickness, all_NO.THICK_MOD, alpha = .2, label = 'This study')
ax.plot(range(700), range(700), '--', c='r')
...
for RID_obs in T.RGI_ID.to_list():
    ...
    RID = get_mosaic_reference(RID_obs)    
    area_thk = rioxr.open_rasterio(home_dir / 'regional_inversion/output/' / RID / 'ex.nc').thk.squeeze()
