...
#for RID in RIDs_with_obs[1:]:
    ...
    vel_smooth = deepcopy(slope)#gauss_filter(vel_ref, 2, 4)
    ...
    bw = 1
    p_save = 100
    p_mb = 1000  # iterations before end when mass balance is recalculated
    s_refresh = 550
    ...
    glacier.config.iceflow_model_lib_path = home_dir / 'regional_inversion/igm/f15_cfsflow_GJ_22_a/{}'.format(resolution)
    ...
    with tf.device(glacier.device_name):
        ...
        beta_update = tf.Variable(0)
        update_surface = tf.Variable(0)
        while p < pmax:
            ...
            def while_body(glacier):
                glacier.update_iceflow()
                glacier.update_t_dt()
                glacier.update_thk()
                glacier.print_info()
                return glacier
            def condition(glacier):
                return tf.math.less(glacier.t, glacier.config.tend)
            tf.while_loop(condition, while_body, [glacier])
